...
def CorrBounds(p, q):
    L = Interval(-((1/p-1)**-1*(1/q-1)**-1)**0.5)
    if L.right<=-1:
        L = L**-1
    U = Interval(((1/q-1)**-1*(1/p-1))**0.5)
    if U.left>=1:
        U = U**-1
    return Interval(L, U).intersection(Interval(-1,1))

def nicegraph(ax):
    ax.set_xlim(0,1)
    ax.set_ylim(0,1)
    ax.set_ylabel('Observed Rate of Coverage')
    ax.set_xlabel(r'Confidence level $1-\alpha$')
    ## Plot the cumulative distribution of the U(0,1) uniform for comparison
    ax.plot([0,1], [0,1], 'k:', label = 'U(0,1)')
    ax.legend()
...
class ECDFStruct():
    def __init__(self, data, origin = 0.5, lbound = None, rbound = None):
...
def CalcA(p, q, rho):
    return (1-p)*(1-q)+rho*((-(p-0.5)**2+0.25)*(-(q-0.5)**2+0.25))**0.5
